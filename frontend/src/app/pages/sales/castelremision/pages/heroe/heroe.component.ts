...
export class HeroeComponent implements OnInit, AfterViewInit {
  ...
  reopenSale(): void {
    const payload = { id: this.heroe.id, status: 'activo' };
    this.repositoryService.putItem('sale', payload).subscribe({
      next: (resp) => { if (resp === 200) this.router.navigate(['castel-remision/' + this.heroe.id + '/edit']); },
  ...
  sendEmailWithPDF = async (pdfBlob: Blob): Promise<void> => {
    ...
    const filename = `castel-remision_${this.heroe.entecode}_${dateStr}.pdf`;
  ...
  private printInfoContent(): void {
    ...
    const fileName = `CASTEL_OrdenRemision#${folio}`;
