...
export class HeroeComponent implements OnInit, AfterViewInit {
  ...
  reopenSale(): void {
    ...
    this.repositoryService.putItem('sale', payload).subscribe({
      next: (resp) => { if (resp === 200) this.router.navigate(['movimiento-de-cotizacion/' + this.heroe.id + '/edit']); },
  ...
  sendEmailWithPDF = async (pdfBlob: Blob): Promise<void> => {
    ...
    const filename = `cotizacion_${this.heroe.entecode}_${dateStr}.pdf`;
  ...
  private printInfoContent(): void {
    ...
    const fileName = `MiSistemaERP_OrdenCotizacion#${folio}`;
