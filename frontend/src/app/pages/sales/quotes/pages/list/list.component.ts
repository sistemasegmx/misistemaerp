...
export class ListComponent implements OnInit, OnDestroy {
  ...
  basestatus = environment.modules.salestatus;
  isLoading = false;
  searchTerm = '';
  selectedStatus = '';
  selectedParent = '';
  ...
  searchModes = [
    ...
    { value: 'entecode', label: 'Código de Cliente' },
    { value: 'entename', label: 'Razón Social' },
  ];

  columnsPDF = [
    { header: 'Folio', dataKey: 'folio' },
    { header: 'País', dataKey: 'countrycode' },
    { header: 'Razón Social', dataKey: 'entename' },
  ...
  loadLS() {
    this.searchHistory = JSON.parse(localStorage.getItem(`searchHistorycotizacion_${this.module}_${this.searchMode}`) || '[]');
    this.favorites = JSON.parse(localStorage.getItem(`searchFavoritescotizacion_${this.module}_${this.searchMode}`) || '[]');
    this.searchMode = localStorage.getItem(`searchModecotizacion_${this.module}`) || 'folio';
  }

  setSearchMode() {
    localStorage.setItem('searchModecotizacion_' + this.module, this.searchMode);
    this.loadData();
  }
  ...
  subscribeToSearch(): void {
    this.searchSubject.pipe(
      ...
      switchMap(({ term }) => {
        ...
        const payload = { [this.searchMode]: trimmed, type: 'cotizacion', limitqty: 10, ascending: 'ASC' };
      ...
      next: (results: ISale[]) => {
        this.searchResults = results.filter((result: ISale) => result.type !== 'castel-cotizacion');
  ...
  addRecentSearch(term: string): void {
    ...
    const key = `searchHistorycotizacion_${this.module}_${this.searchMode}`;
  ...
  selectSuggestion(item: any): void {
    const suggestion = String(item?.[this.searchMode] || '').trim();
    if (!suggestion) return;
    this.preventReopen = true;
    this.searchTerm = suggestion.substring(0, 30);
    this.toggleDropdown(true);
    this.addRecentSearch(this.searchTerm);
    this.searchHistory = JSON.parse(localStorage.getItem(`searchHistorycotizacion_${this.module}_${this.searchMode}`) || '[]');
  ...
  updateFavorites(term: string, add: boolean): void {
    ...
    localStorage.setItem(`searchFavoritescotizacion_${this.module}_${this.searchMode}`, JSON.stringify(this.favorites));
    this.preventReopen = true;
  ...
  clearSearchHistory(): void {
    localStorage.removeItem(`searchHistorycotizacion_${this.module}_${this.searchMode}`);
    this.searchHistory = [];
    this.clearSearch();
    this.loadData();
  }
  ...
  loadSeller(): void { this.repository.getAllData('user').subscribe((resp: IUser[]) => { this.allSeller = resp; }); }
