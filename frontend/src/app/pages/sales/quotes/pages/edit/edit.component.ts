...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  private loadHeroeStatus(id: string): void {
    this.repositoryService.getOneById(this.module, { id }).subscribe({
      next: (resp: ISale) => {
        if (resp.status === 'inactivo' || resp.status === 'finalizada') { this.router.navigate(['movimiento-de-cotizacion/' + resp.id]); }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.repositoryService.deleteItem(this.module, this.heroe.id).subscribe({
          next: () => this.router.navigate(['movimiento-de-cotizacion']),
