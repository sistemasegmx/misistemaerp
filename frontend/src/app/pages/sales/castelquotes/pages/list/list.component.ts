...
export class ListComponent implements OnInit, OnDestroy {
  ...
  loadLS() {
    this.searchHistory = JSON.parse(localStorage.getItem(`searchHistorycastel-cotizacion_${this.module}_${this.searchMode}`) || '[]');
    this.favorites = JSON.parse(localStorage.getItem(`searchFavoritescastel-cotizacion_${this.module}_${this.searchMode}`) || '[]');
    this.searchMode = localStorage.getItem(`searchModecastel-cotizacion_${this.module}`) || 'folio';
  }

  setSearchMode() {
    localStorage.setItem('searchModecastel-cotizacion_' + this.module, this.searchMode);
    this.loadData();
  }
  ...
  subscribeToSearch(): void {
    this.searchSubject.pipe(
      ...
      switchMap(({ term }) => {
        ...
        const payload = { [this.searchMode]: trimmed, type: 'castel-cotizacion', limitqty: 10, ascending: 'ASC' };
  ...
  addRecentSearch(term: string): void {
    ...
    const key = `searchHistorycastel-cotizacion_${this.module}_${this.searchMode}`;
  ...
  updateFavorites(term: string, add: boolean): void {
    ...
    localStorage.setItem(`searchFavoritescastel-cotizacion_${this.module}_${this.searchMode}`, JSON.stringify(this.favorites));
    this.preventReopen = true;
  ...
  clearSearchHistory(): void {
    localStorage.removeItem(`searchHistorycastel-cotizacion_${this.module}_${this.searchMode}`);
    this.searchHistory = [];
    this.clearSearch();
    this.loadData();
  }
