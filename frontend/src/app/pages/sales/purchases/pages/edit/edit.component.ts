...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (this.itemId) { this.loadHeroeStatus(this.itemId); }
    });
    this.formSubscription = this.letsdoitForm.valueChanges.subscribe(changes => {
      Object.assign(this.heroe, this.letsdoitForm.value);
    });
  }
  ...
  private createForm(): FormGroup {
    return this.formBuilder.group({
      ...
      trackingnumber: ['', []],
      entecode: [''],
      entename: ['Proveedor', [Validators.required]],
      entecontact: [''],
  ...
  private loadHeroeStatus(id: string): void {
    this.repositoryService.getOneById(this.module, { id }).subscribe({
      next: (resp: ISale) => {
        if (resp.status === 'inactivo' || resp.status === 'finalizada') {
          this.router.navigate(['movimiento-de-compra/' + resp.id]);
        } else {
          this.heroe = resp;
  ...
  private loadAdditionalData(): void {
    forkJoin({
      ...
      entes: this.repositoryService.getAllDataFiltered('ente', { 'type': 'proveedor' })
    }).subscribe({
  ...
  saveItem(): void {
    ...
    const { enteid, enteorder, trackingnumber, entecode, entename, entecontact, currency, parity, taxname, taxamount, pricelevel, salename, validity, startdate, enddate, defaultcomment, description, status } = this.letsdoitForm.value;
    const payload: {} = { ...this.heroe, enteid, enteorder, trackingnumber, entecode, entename, entecontact, currency, parity, taxname, taxamount, pricelevel, salename, validity, startdate, enddate, defaultcomment, description, status };
  ...
  searchEnte(): void {
    if (!this.allEntes?.length) { console.warn('No hay proveedores cargados en allEntes.'); return; }
    const query = this.enteSearch?.trim().toLowerCase() || '';
    this.searchResults = query
      ? this.allEntes.filter(({ code, fullname, rfc, fiscalname }) =>
        [code, fullname, rfc, fiscalname]
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.repositoryService.deleteItem(this.module, this.heroe.id).subscribe({
          next: () => this.router.navigate(['movimiento-de-compra']),
