...
export class AddComponent implements OnInit, AfterViewInit {
  ...
  private loadData(): void {
    forkJoin({
      ...
      entes: this.repositoryService.getAllDataFiltered('ente', { 'type': 'proveedor', 'status': 'activo' })
    }).subscribe({
  ...
  private createItem(item: ISale): void {
    this.repositoryService.postItem(this.module, item).subscribe({
      next: (resp: ISale) => { this.router.navigate(['movimiento-de-compra/' + resp.id + '/edit']); },
  ...
  searchEnte(): void {
    const query = this.letsdoitForm.get('enteSearch')?.value.trim().toLowerCase();
    this.searchResults = this.allEntes.filter(({ code, fullname, rfc, fiscalname }) =>
