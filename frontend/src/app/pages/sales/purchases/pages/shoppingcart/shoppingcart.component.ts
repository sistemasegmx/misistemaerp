...
export class ShoppingCartComponent implements OnInit, AfterViewInit {
  ...
  addItemToCart(item: IItem, quantity: number = 1): void {
    ...
    const convertedCost = this.convertCurrency(+item.cost, item.currencycost);
    const levelFactor = (+this.pricelevel.amount || 0) / 100;
    const modifiedCost = +(convertedCost * (1 + levelFactor)).toFixed(2);
    ...
    if (existingItem) {
      ...
      existingItem.cost = modifiedCost;
      existingItem.price = modifiedCost;
  ...
  changeStatusSale(newStatus: string) {
    const payload = { id: this.sale.id, status: newStatus };
    this.repositoryService.putItem('sale', payload).pipe(
      switchMap(() => {
        const currentStatus = this.salestatus.find(s => s.status === newStatus);
      ...
      next: () => {
        ...
        this.cdr.detectChanges();
        this.router.navigate(['movimiento-de-compra/' + this.sale.id]);
      },
  ...
  closeSale(): void {
    this.repositoryService.putItem('sale', { id: this.sale.id, status: 'finalizada' }).subscribe({
      next: (resp) => resp === 200 && this.repositoryService.postItem('salehistory', {
        saleid: this.sale.id, status: 'finalizada',
        description: this.salestatus.find(s => s.status === 'finalizada')?.description || '',
        module: 'orden', action: 'status'
      }).subscribe(() => (this.printInfo(), this.router.navigate(['movimiento-de-compra/' + this.sale.id]))),
  ...
  sendEmailWithPDF = async (pdfBlob: Blob): Promise<void> => {
    ...
    const filename = `compra_${this.sale.entecode}_${dateStr}.pdf`;
