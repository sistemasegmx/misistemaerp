...
export class ListComponent implements OnInit, OnDestroy {
  ...
  loadLS() {
    this.searchHistory = JSON.parse(localStorage.getItem(`searchHistoryremision_${this.module}_${this.searchMode}`) || '[]');
    this.favorites = JSON.parse(localStorage.getItem(`searchFavoritesremision_${this.module}_${this.searchMode}`) || '[]');
    this.searchMode = localStorage.getItem(`searchModeremision_${this.module}`) || 'folio';
  }

  setSearchMode() {
    localStorage.setItem('searchModeremision_' + this.module, this.searchMode);
    this.loadData();
  }
  ...
  subscribeToSearch(): void {
    this.searchSubject.pipe(
      ...
      switchMap(({ term }) => {
        ...
        const payload = { [this.searchMode]: trimmed, type: 'remision', limitqty: 10, ascending: 'ASC' };
      ...
      next: (results: ISale[]) => {
        this.searchResults = results.filter((result: ISale) => result.type !== 'castel-remision');
  ...
  addRecentSearch(term: string): void {
    ...
    const key = `searchHistoryremision_${this.module}_${this.searchMode}`;
  ...
  updateFavorites(term: string, add: boolean): void {
    ...
    localStorage.setItem(`searchFavoritesremision_${this.module}_${this.searchMode}`, JSON.stringify(this.favorites));
    this.preventReopen = true;
  ...
  clearSearchHistory(): void {
    localStorage.removeItem(`searchHistoryremision_${this.module}_${this.searchMode}`);
    this.searchHistory = [];
    this.clearSearch();
    this.loadData();
  }
