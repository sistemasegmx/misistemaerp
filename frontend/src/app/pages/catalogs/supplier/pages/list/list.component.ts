...
export class ListComponent implements OnInit, OnDestroy {
  ...
  loadLS() {
    this.searchHistory = JSON.parse(localStorage.getItem(`searchHistoryproveedor_${this.module}_${this.searchMode}`) || '[]');
    this.favorites = JSON.parse(localStorage.getItem(`searchFavoritesproveedor_${this.module}_${this.searchMode}`) || '[]');
    this.searchMode = localStorage.getItem(`searchModeproveedor_${this.module}`) || 'code';
  }

  setSearchMode() {
    localStorage.setItem('searchModeproveedor_' + this.module, this.searchMode);
    this.loadData();
  }
  ...
  subscribeToSearch(): void {
    this.searchSubject.pipe(
      ...
      switchMap(({ term }) => {
        ...
        const payload = { [this.searchMode]: trimmed, type: 'proveedor', limitqty: 10, ascending: 'ASC' };
  ...
  addRecentSearch(term: string): void {
    ...
    const key = `searchHistoryproveedor_${this.module}_${this.searchMode}`;
  ...
  updateFavorites(term: string, add: boolean): void {
    ...
    localStorage.setItem(`searchFavoritesproveedor_${this.module}_${this.searchMode}`, JSON.stringify(this.favorites));
    this.preventReopen = true;
  ...
  clearSearchHistory(): void {
    localStorage.removeItem(`searchHistoryproveedor_${this.module}_${this.searchMode}`);
    this.searchHistory = [];
    this.clearSearch();
    this.loadData();
  }
