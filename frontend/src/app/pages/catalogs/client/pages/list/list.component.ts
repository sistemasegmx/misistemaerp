...
export class ListComponent implements OnInit, OnDestroy {
  ...
  searchCache: { [key: string]: { data: IEnte[], timestamp: number } } = {};
  ...
  columnsPDF = [
    ...
    { header: 'Razón Social', dataKey: 'fiscalname' },
  ...
  loadLS() {
    this.searchHistory = JSON.parse(localStorage.getItem(`searchHistorycliente_${this.module}_${this.searchMode}`) || '[]');
    this.favorites = JSON.parse(localStorage.getItem(`searchFavoritescliente_${this.module}_${this.searchMode}`) || '[]');
    this.searchMode = localStorage.getItem(`searchModecliente_${this.module}`) || 'code';
  }

  setSearchMode() {
    localStorage.setItem('searchModecliente_' + this.module, this.searchMode);
    this.loadData();
  }
  ...
  subscribeToSearch(): void {
    this.searchSubject.pipe(
      ...
      switchMap(({ term }) => {
        ...
        const payload = { [this.searchMode]: trimmed, type: 'cliente', limitqty: 10, ascending: 'ASC' };
  ...
  addRecentSearch(term: string): void {
    ...
    const key = `searchHistorycliente_${this.module}_${this.searchMode}`;
  ...
  updateFavorites(term: string, add: boolean): void {
    ...
    localStorage.setItem(`searchFavoritescliente_${this.module}_${this.searchMode}`, JSON.stringify(this.favorites));
    this.preventReopen = true;
  ...
  clearSearchHistory(): void {
    localStorage.removeItem(`searchHistorycliente_${this.module}_${this.searchMode}`);
    this.searchHistory = [];
    this.clearSearch();
    this.loadData();
  }
