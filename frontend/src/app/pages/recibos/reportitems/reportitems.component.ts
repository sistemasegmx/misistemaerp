...
export class ReportItemsComponent implements OnInit, OnDestroy {
  ...
  allowedTypes = [
    { value: 'bodega-usa', label: 'Bodega USA' },
    { value: 'proceso-importacion', label: 'Proceso de Importación' },
    { value: 'bodega-tijuana', label: 'Bodega Tijuana' }
  ];
  ...
  columnsPDF = [
    ...
    { header: 'Compra', dataKey: 'originfolio' },
    { header: 'No. Parte', dataKey: 'code' },
    { header: 'Descripción', dataKey: 'description' },
    { header: 'Recibida', dataKey: 'qty' },
    { header: 'Recibo', dataKey: 'folio' },
  ...
  loadData(): void {
    ...
    this.repository.getAllDataPaginatedBySaleItemsRecibo(this.module, payload).subscribe((resp) => {
  ...
  getQtyClass(type: string): string {
    const colorMap: Record<string, string> = {
      'bodega-usa': 'text-primary',
      'proceso-importacion': 'text-success',
      'bodega-tijuana': 'text-warning'
    };
    return colorMap[type] || 'text-muted';
  }

  getReceivedQty(item: ISaleItems): number {
    const typeMap: Record<string, keyof ISaleItems> = {
