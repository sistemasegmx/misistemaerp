...
export class AddComponent implements OnInit, AfterViewInit {
  ...
  private loadHeroeData(id: string): void {
    this.repositoryService.getOneById(this.module, { id }).subscribe((resp: IPricelevel) => {
  ...
  private updateItem(item: IPricelevel): void {
    this.repositoryService.putItem(this.module, item).subscribe({
      next: () => this.router.navigate(['configuracion-de-precios']),
      error: () => Swal.fire('Error', 'No se pudo actualizar el Registro', 'error')
    });
  }

  private createItem(item: IPricelevel): void {
    this.repositoryService.postItem(this.module, item).subscribe({
      next: (resp: IPricelevel) => { this.router.navigate(['configuracion-de-precios']); },
