...
export class ListComponent implements OnInit, OnDestroy {
  ...
  allData: ITax[] = [];
  allDataFiltered: ITax[] = [];

  columnsPDF = [
    { header: 'Nombre', dataKey: 'fullname' },
    { header: 'Descripción', dataKey: 'description' },
    { header: 'Tasa (%)', dataKey: 'amount' },
  ...
  ngOnInit(): void {
    this.routerSubscription = this.router.events.subscribe(event => { if (event instanceof NavigationEnd && !event.urlAfterRedirects.startsWith('/configuracion-de-impuestos')) { this.searchService.changeSearch(''); } });
